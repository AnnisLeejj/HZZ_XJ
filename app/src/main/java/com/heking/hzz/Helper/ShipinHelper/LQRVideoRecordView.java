...
/**
 * CSDN_LQR Created by lee on 2017/10/16.
 * <p>
 * 视频录制控件
 */
public class LQRVideoRecordView extends SurfaceView implements MediaRecorder.OnErrorListener {
    ...
    Camera.AutoFocusCallback myAutoFocusCallback;//自动对焦
    ...
    public LQRVideoRecordView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mXpx = typedArray.getInteger(R.styleable.LQRVideoRecordView_vrv_x_px, 1280);// 默认640
        mYpx = typedArray.getInteger(R.styleable.LQRVideoRecordView_vrv_y_px, 720);// 默认480
        mOutFormat = typedArray.getInteger(R.styleable.LQRVideoRecordView_vrv_out_format, 0);//默认0:mp4     1:3gp
        mRecordMaxTime = typedArray.getInteger(R.styleable.LQRVideoRecordView_vrv_record_max_time, 30);//默认最长10秒
        mVideoEncodingBitRate = typedArray.getInteger(R.styleable.LQRVideoRecordView_vrv_video_encoding_bit_rate, 5 * 1024 * 1024);
        ...
        myAutoFocusCallback = new Camera.AutoFocusCallback() {
            ...
            public void onAutoFocus(boolean success, Camera camera) {
                if (success) {
                    ...
                    //Toast.makeText(context, "自动聚焦成功", Toast.LENGTH_SHORT).show();
                } else {
                    //Toast.makeText(context, "自动聚焦失败", Toast.LENGTH_SHORT).show();
                }
            }
        };
    }
    ...
    public void openCamera() {
        try {
            ...
            LogUtils.w("shipin", "预览的各种分辨率", supportedPreviewSizes);
            LogUtils.w("shipin", "摄像头支持的各种分辨率", supportedPictureSizes);
...
//            mCamera.setParameters(params);
    ...
    private void initRecord() throws IOException {
        ...
        try {
            ...
            mMediaRecorder.setMaxDuration(mRecordMaxTime * 1000);
